...
const useLobby = (username: string) => {
    ...
    const [removeUserFromLobby] = useMutation(REMOVE_USER_FROM_LOBBY, { onError: ({ graphQLErrors }) => handleNotification((graphQLErrors[0].message || 'Something went wrong'), 5) });
    ...
    window.onbeforeunload = () => {
        removeUserFromLobby({ variables: { username: username }});
    };
